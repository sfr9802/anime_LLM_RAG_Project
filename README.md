...
LLM 단독 사용 시 **비주류 애니메이션이나 세부 설정에 대해 높은 확률로 할루시네이션이 발생**하는 문제를 해결하기 위해,  
이 프로젝트에서는 **나무위키 기반 문서 벡터 검색 + LLM 응답 구조**를 직접 구축했습니다.  
사용자는 대화형 UI를 통해 **애니 관련 질문**을 입력하면,  
나무위키 문서 기반 벡터 검색 + LLM 조합으로 **문서 인용과 답변**을 받을 수 있습니다.

> 예: "신이 된 히로인의 서사가 있는 애니메이션은?" → "스즈미야 하루히의 우울"  
> LLM 단독으로는 정확한 응답이 어렵지만, 벡터 검색 기반 RAG 구조에서 안정적 인용 가능.
...
- **도메인 특화 데이터 수집 및 정제**  
  - 2006~2025년 나무위키에서 수집한 7,700여 개 애니메이션 관련 문서  
  - 등장인물/설정/스토리/평가 등 **하위 항목까지 재귀 수집 + 정제**

- **검색 품질 최적화**  
  - 검색 정확도 `Hit@8: 84.2%`, 문서 중복률 `0.0%`  
  - 검색+응답 평균 응답시간 `p95: 178ms`  
  - ※ title 단위 평가 기준(by=title, distinct_by=title)

- **로컬 GPU 환경에서 벡터 검색 + LLM 추론까지 통합 실행**  
  - Docker Compose 기반 FastAPI + Chroma + LLM + Redis 구성  
  - OAuth2 로그인, JWT 발급, Redis 기반 인증 흐름 구축 포함
...
![anime_rag_architecture](./image/anime_rag_architecture.png)
...
- OAuth2 인증용 Google Cloud Client ID/Secret이 필요합니다.  
  → `application.yml` 또는 `.env`에 환경변수로 설정
