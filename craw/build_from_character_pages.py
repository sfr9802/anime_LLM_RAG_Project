# build_from_character_pages_refactored.py
# 사용 예:
#   python build_from_character_pages_refactored.py --input D:\port\craw\crawled.jsonl --out D:\port\craw\out_with_chars.jsonl --summarize --top-summary --to-mongo --to-chroma --embed-device cpu

from __future__ import annotations
import argparse, os, json, re, hashlib, unicodedata, logging
...
def recover_sections_from_body(sections: dict) -> dict:
    ...
    if not isinstance(body, dict): return sections
...
def looks_non_character(name: str) -> bool:
    ...
    if base in {s.lower() for s in NON_CHAR_NAMES}: return True
    if any(k in name for k in NON_CHAR_NAMES): return True
    return False
...
def _llm_chat_json(prompt: str, timeout: float | None = None) -> Dict[str, Any]:
    import requests, json as _json, re as _re
    SMART = str.maketrans({"“": '"', "”": '"', "„": '"', "‟": '"', "‘": "'", "’": "'", "‚": "'", "‛": "'"})
    FENCE_RE = _re.compile(r"^```(?:json)?\s*|\s*```$", _re.IGNORECASE | _re.MULTILINE)

    def _strip_code_fence(s: str) -> str: return FENCE_RE.sub("", s)
    def _remove_trailing_commas(s: str) -> str: return _re.sub(r",(\s*[}\]])", lambda m: m.group(1), s)
    def _escape_newlines_inside_strings(s: str) -> str:
        out = []; in_str = False; esc = False
        for ch in s:
            if in_str:
                if esc: esc = False
                elif ch == "\\": out.append(ch) or setattr(__import__('builtins'),'_',True)  # no-op to satisfy lints
    ...
    def _try_salvage(raw: str) -> dict:
        m_sum = _re.search(r'"summary"\s*:\s*"((?:[^"\\]|\\.)*)"', raw, _re.S)
        m_arr = _re.search(r'"bullets"\s*:\s*\[(.*?)\]', raw, _re.S)
        bullets = []
        if m_arr:
            bullets = _re.findall(r'"((?:[^"\\]|\\.)*)"', m_arr.group(1), _re.S)
        return {"summary": (m_sum.group(1) if m_sum else "").strip(), "bullets": [b.strip() for b in bullets]}

    payload = {
        "model": LLM_MODEL,
        "messages": [
            {"role": "system",
             "content": ('오직 RFC8259 호환 JSON **객체 하나만** 출력:\n'
                         '{"summary": "string", "bullets": ["point1", "point2", "point3"]}\n'
                         '- 기타 텍스트 금지, 줄바꿈은 문자열 내에서 \\n')},
    ...
    try:
        ...
        raw = _strip_code_fence(txt).translate(SMART)
        prepped = _escape_newlines_inside_strings(_remove_trailing_commas(raw))
        cands = _extract_candidate_jsons(prepped)
        cands.sort(key=lambda x: (('"summary"' in x) + ('"bullets"' in x) - ('"point1"' in x) - ('"string"' in x), len(x)), reverse=True)
        for blk in cands:
            try: return _json.loads(blk)
            except Exception: pass
        salv = _try_salvage(raw)
        if salv.get("summary") or salv.get("bullets"): return salv
        raise RuntimeError("LLM JSON parse error")
    except Exception as e:
        LOG.error("LLM JSON parse fail: %s ...", (raw or "")[:800])
...
def summarize_kor(seed: str, text: str, bullets: int, max_chars: int) -> Dict[str, Any]:
    ...
    if not s and bl: s = bl[0]
    return {"summary": s, "bullets": bl, "model": LLM_MODEL, "ts": datetime.now(_tz).isoformat()}

# ------------------------- Vector/Mongo sinks (단일 파일 내 구현) -------------------------
class STEmbedder:
    ...
    def encode(self, texts: List[str]) -> List[List[float]]:
        import numpy as np
        if not texts: return []
        arr = self.model.encode(texts, batch_size=self.batch_size, convert_to_numpy=True, normalize_embeddings=True, show_progress_bar=False)
...
class MongoSink:
    ...
    def ensure_indexes(self):
        self.pages.create_index([("_id", self._ASC)], unique=True, name="pk_doc_id")
        self.pages.create_index([("seed", self._ASC)], name="idx_seed")
        self.pages.create_index([("title", self._ASC)], name="idx_title")
        self.pages.create_index([("created_at", self._ASC)], name="idx_created_at")
        self.pages.create_index([("vectorized", self._ASC)], name="idx_vectorized")
        self.chunks.create_index([("_id", self._ASC)], unique=True, name="pk_chunk_id")
        self.chunks.create_index([("doc_id", self._ASC), ("seg_index", self._ASC)], unique=True, name="uk_doc_seg")
        self.chunks.create_index([("section", self._ASC)], name="idx_section")
    ...
    def mark_vectorized(self, doc_id: str, seg_ids: List[str]):
        self.pages.update_one(
            {"_id": doc_id},
            {"$set": {"vectorized": True, "vector_ts": datetime.now(_tz).isoformat(), "chroma_segment_ids": seg_ids}}
        )
...
def main():
    ...
    ap.add_argument("--sum-max-chars", type=int, default=6000, help="LLM 입력 최대 길이(문자)")
    ...
    ap.add_argument("--embed-device", type=str, default="cpu", choices=["cpu","cuda"], help="임베딩 디바이스(cpu 권장)")
    ap.add_argument("--no-file-out", action="store_true", help="JSONL 파일 저장 생략")
    # 보일러 컷 옵션
    ...
    for r in iter_jsonl(args.input):
        ...
        if section == "등장인물" and title and title != seed:
            if looks_non_character(proper_title or title): continue
            desc = "\n\n".join(chunks).strip()
    ...
    def write_per_seed(path_dir: str, seed: str, doc: Dict[str, Any]):
        safe = re.sub(r'[\\/:*?"<>|]', '_', seed)[:180] + ".jsonl"
        with open(os.path.join(path_dir, safe), "w", encoding="utf-8") as g:
    ...
    for seed in sorted(set(list(sections_bag.keys()) + list(characters_bag.keys()))):
        ...
        for s in ordered:
            ...
            for t in secmap[s]["chunks"]:
                if boiler and t in boiler: continue
                ...
                if not joined:
                    for k, v in sections.items():
                        if isinstance(v, dict) and v.get("text"):
                            joined.append(f"[{k}] {v['text']}")
                full_text = "\n\n".join(joined)
                sres = summarize_kor(seed, full_text, bullets=args.sum_bullets, max_chars=args.sum_max_chars) if full_text.strip() else {}
                if sres and (sres.get("summary") or sres.get("bullets")):
                    ytxt = (sres.get("summary") or "").strip()
                    ybul = [b.strip() for b in (sres.get("bullets") or []) if b and b.strip()]
                    sections["요약"] = {
                        "text": ytxt,
                        "bullets": ybul,
                        "chunks": ([ytxt] if ytxt else []) + ([f"- {b}" for b in ybul] if ybul else []),
                        "urls": [], "model": sres.get("model"), "ts": sres.get("ts"),
                    }
        ...
        raw_len = sum(len(s) for s in flat_segments)
        sum_txt = (sections.get("요약") or {}).get("text","") or ""
        sum_len = len(sum_txt)
        comp = round((sum_len / raw_len), 4) if raw_len > 0 and sum_len > 0 else None
        ...
        ids = texts = metas = []
        if mongo:
            mongo.upsert_page(doc, n_segments)
            ids, texts, metas = mongo.upsert_chunks(doc)  # 요약 제외 section 기반

        if chroma and n_segments > 0:
            seg_ids = chroma.upsert_embeddings(ids, texts, metas)  # CPU 임베딩
            if mongo:
        ...
        if args.per_seed_dir:
            write_per_seed(args.per_seed_dir, seed, doc)
    ...
    if out_f: LOG.info("Output JSONL: %s", args.out)
    if args.per_seed_dir:
