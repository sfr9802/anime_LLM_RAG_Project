...
config = _SettingsCls()
from app.domain.embeddings import embed_passages  # 반환: np.ndarray 또는 list 지원 권장
from app.infrastructure.vector.chroma_store import upsert as chroma_upsert
from app.infrastructure.mongo.mongo_client import get_db  # db = get_db()

from app.domain.chunker import window_by_chars
...
def ingest_v2_jsonl(
    ...
    *,
    ...
    db = get_db()
    works = db[ config.mongo_raw_col ]
    chars = db[ config.mongo_chunk_col ]
