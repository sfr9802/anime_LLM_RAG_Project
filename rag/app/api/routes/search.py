...
from app.api.schemas.responses import SearchResponse, SearchResult
from app.infrastructure.vector import search_vectors  # __init__.py에서 export한 함수
...
async def experimental_search(req: SearchRequest) -> SearchResponse:
    ...
    for h in hits:
        if not isinstance(h, dict):
            # 방어적으로 문자열화
            results.append(
                SearchResult(doc_id="", content=str(h), score=None)
            )
            continue
