...
public class UserProfileService {
    ...
    public UserProfileResDto upsertMyProfile(Long userId, UserProfileReqDto req) {
        AppUser user = appUserRepository.findById(userId).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "user not found"));
        UserProfile profile = userProfileRepository.findByAppUser(user).orElseGet(() -> UserProfile.builder().appUser(user).nickname(defaultNick(user.getEmail())).build());
        if (req.getNickname() != null && !req.getNickname().isBlank()) profile.changeNickname(req.getNickname().trim());
