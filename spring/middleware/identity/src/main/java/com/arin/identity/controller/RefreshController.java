...
public class RefreshController {
    ...
    public ResponseEntity<?> refreshAccessToken(HttpServletRequest request,
        ...
        String refreshToken = (rtFromCookie != null && !rtFromCookie.isBlank()) ? rtFromCookie : extractToken(request);
        if (refreshToken == null || refreshToken.isBlank()) {
            return ResponseEntity.badRequest().headers(noCache).body(Map.of("error","BAD_REQUEST","message","refresh token missing"));
        }
        Claims claims;
        try { claims = jwtProvider.getClaims(refreshToken); }
        catch (Exception e) { return ResponseEntity.status(401).headers(noCache).body(Map.of("error","UNAUTHORIZED","message","invalid refresh token")); }
        if (!"ref".equals(jwtProvider.getType(refreshToken))) {
            return ResponseEntity.status(401).headers(noCache).body(Map.of("error","UNAUTHORIZED","message","not a refresh token"));
        }
        ...
        if (!ok) {
            ...
            return ResponseEntity.status(401).headers(noCache).body(Map.of("error","UNAUTHORIZED","message","refresh reuse detected"));
        }
        ...
        return ResponseEntity.ok().headers(noCache).header(HttpHeaders.SET_COOKIE, cookie.toString())
                .body(new TokenResponseDto(newAccess, atExpS));
    }
